...
#
# ConvertTo-BatchSafe above strips % ! ^ & -- and all four are LEGAL in Windows
# paths. Running a path through it does not fail loudly; it silently hands back
# a DIFFERENT path. A sidecar template named 'Qwen3 & Friends.jinja', or any
# model living under a folder like 'Models & Templates', or a user whose account
# name contains '!', comes out the other side pointing at a file that does not
# exist -- and the symptom is llama-server refusing a template that is plainly
# sitting there. Reported by John McCardle against fileserver.ps1's twin of this
# function; fileserver.ps1 grew ConvertTo-CmdPathSafe to fix it and this file
# never got the matching half.
#
# The security requirement is unchanged and is fully met by less. These values
# land inside `set "VAR=..."` in a generated .cmd that launch.bat CALLs, so what
# has to be impossible is escaping those double quotes or starting a new line.
# That is exactly two characters -- '"' and CR/LF -- and both are ILLEGAL in
# Windows filenames, so removing them cannot cost a legitimate path anything.
#
# Note this file's generated .cmd does not enable delayed expansion either, so a
# bare '!' is literal there, the same reasoning ConvertTo-CmdPathSafe records.
function ConvertTo-BatchPathSafe([string]$s) {
    if (-not $s) { return '' }
    return (($s -replace '[\r\n]', ' ') -replace '"', '').Trim()
}
...
if ($Emit -eq 'batch') {
    ...
    $setLines = @(
        ...
        ('set "MODEL_CHAT_TEMPLATE_FILE=' + (ConvertTo-BatchPathSafe $info.chatTemplateFile) + '"'),
