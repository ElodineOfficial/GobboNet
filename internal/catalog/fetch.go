...
func writeCache(dir string, c *cacheFile) error {
	...
	// then fails to parse on every subsequent run. atomicfile gives each writer
	// its own temp file, so a refresh racing another one cannot fail on the
	// rename.
	return atomicfile.Write(filepath.Join(dir, cacheName), b, 0o600)
}
