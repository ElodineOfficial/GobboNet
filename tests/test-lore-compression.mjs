...
function build() {
  ...
  const ctx = {
    ...
    // Lives in js/02-model.js, which loads first in the browser. Stubbed here
    // for the same reason activeModel and escapeHtml are: this harness runs
    // 07-prompt.js on its own.
    getRequestModelName: () => 'local',
