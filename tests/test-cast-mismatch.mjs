...
// The strip is no longer shown by toggling display: the container is a
// permanent live region and stays in the DOM whether or not it has anything to
// say (see A11Y-1). "Shown" therefore means "has content".
const shown = el => el.innerHTML !== '';
...
console.log('\n=== F. it can actually be perceived ===');
{
  // The strip's whole job is "read this before you press send". A screen reader
  // user who is not told has walked straight into issue #19 (A11Y-1).
  ok(/id="cast-mismatch"[^>]*role="status"/.test(CHAT_HTML) ||
     /role="status"[^>]*id="cast-mismatch"/.test(CHAT_HTML),
    'the container is a live region (role="status")');

  // A container that is display:none when content arrives is unreliable across
  // screen readers -- several will not fire at all. It has to stay in the DOM.
  const tag = (CHAT_HTML.match(/<div[^>]*id="cast-mismatch"[^>]*>/) || [''])[0];
  ok(!/display:\s*none/.test(tag), 'the live region is never display:none');

  ok(!/el\.style\.display/.test(mismatchFn),
    'updateCastMismatch does not toggle display on the live region');

  // render() runs on nearly every interaction. Rewriting identical content
  // would make a screen reader repeat itself endlessly.
  ok(/el\.innerHTML !== html/.test(mismatchFn),
    'identical content is not rewritten, so it is not re-announced');

  const el = run({
    thread: { id: 't', cardId: 'cb', messages: [{ role: 'assistant', content: 'x', cardId: 'cb' }] },
    activeCardId: 'ca',
  });
  ok(/aria-label="Switch back to CodeGoblin"/.test(el.innerHTML),
    'the switch control names its destination for a screen reader');
  ok(/<button /.test(el.innerHTML),
    'the switch control is a real button, so it is keyboard reachable');
  ok(/cast-mismatch-dot[^>]*aria-hidden="true"/.test(el.innerHTML),
...
console.log('\n=== G. it is wired in ===');
{
  ...
  ok(!/castStrip/.test(DASH), 'updateInputState does not hide the live region');
  ok(/\.cast-mismatch-live\s*\{/.test(CSS), 'the live wrapper is styled to occupy nothing when empty');
