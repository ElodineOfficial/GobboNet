...
function safeImageUrl(value) {
  ...
  if (/^blob:/i.test(s))   return safeUrlBody(s) && sameOriginBlob(s) ? s : '';
  if (/^https?:\/\//i.test(s)) {
    if (!safeUrlBody(s)) return '';
    return (typeof state !== 'undefined' && state && state.settings && state.settings.allowRemoteImages) ? s : '';
  }
  return '';
}

/* Whatever safeImageUrl returns has to be safe to drop into a src="" or a CSS
   url() WITHOUT the caller adding anything.

   The data: branch above already guarantees that: it is anchored end to end
   with an explicit character class, so nothing but base64 can survive it. The
   other two branches only tested their SCHEME -- /^blob:/i and /^https?:\/\//i,
   neither anchored -- and then returned the entire rest of the string
   unexamined. Reported by John McCardle against the blob branch; the http one
   is the same defect and is fixed here too rather than left as its twin.

   Nothing is exploitable through today's call sites, because every one of them
   escapes on the way out: escapeHtml for src attributes, quote-and-backslash
   escaping for the CSS background. But the function is NAMED safeImageUrl and
   is the designated gate, so a future caller that reasonably trusts its output
   would inherit an unvalidated string. Closing it here means the contract the
   name implies is the contract it keeps.

   Only characters that can terminate a URL context are refused: whitespace and
   controls, the two quote marks, angle brackets, and backslash. Parentheses,
   '&', '?', '%' and '=' stay legal -- they appear in real image URLs, and a
   quoted CSS string tolerates them. */
function safeUrlBody(s) {
  return !/[\s"'<>\\]|[\u0000-\u001f\u007f]/.test(s);
}

/* A blob: URL is produced locally by URL.createObjectURL, so its shape is
   blob:<origin>/<uuid>. One carrying somebody else's http(s) origin did not
   come from this page, and the browser will refuse to resolve it anyway --
   so it is a string pretending to be an image, which is the thing this gate
   exists to catch.

   blob:null/... is left alone: that is what file:// mode produces, and the
   attachment path depends on it. When there is no location to compare against
   (the test harness, a worker) the check abstains rather than guessing. */
function sameOriginBlob(s) {
  const m = /^blob:(https?:\/\/[^/]+)\//i.exec(s);
  if (!m) return true;
  const here = (typeof location !== 'undefined' && location && location.origin) ? location.origin : '';
  if (!here) return true;
  return m[1].toLowerCase() === here.toLowerCase();
}
