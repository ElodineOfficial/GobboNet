...
function updateInputState() {
  ...
  // The cast-mismatch strip is not touched here. It is a live region that has
  // to stay in the DOM and unhidden to work at all, and an empty one occupies
  // no space, so there is nothing to hide. updateCastMismatch() clears it when
  // there is no thread.
...
function updateCastMismatch() {
  ...
  // Writing the same sentence again would make a screen reader read it again,
  // and render() runs on nearly every interaction. Only touch the DOM when the
  // message actually changes.
  const show = (html) => { if (el.innerHTML !== html) el.innerHTML = html; };
  const hide = () => show('');
  ...
  const button = canSwitch
    ? `<button class="cast-mismatch-btn" onclick="activateCard('${escapeJsAttr(previous.id)}')"
         aria-label="Switch back to ${escapeHtml(previous.name)}"
         title="Switch back to ${escapeHtml(previous.name)}">Switch</button>`
    : '';

  // aria-hidden on the dot: it is decoration, and "circle" read aloud before
  // every announcement is noise.
  show(
    `<div class="cast-mismatch">` +
    `<span class="cast-mismatch-dot" aria-hidden="true"></span>` +
